...
function PipelineListing() {
  ...
  }, [selectedKey, currentPage, pageSize, searchText]);
  ...
  useEffect(() => {
    const filtered = pipelines.filter((pipeline) =>
      pipeline.name.toLowerCase().includes(searchText.toLowerCase())
    );
  ...
  useEffect(() => {
    ...
    setToast({
      message: "Could not delete pipeline",
  ...
  const handleDelete = async () => {
    if (selectedItems.length === 1) {
      ...
      const remainingItems = pipelines.filter(
    ...
    <div>
      ...
      <Pivot selectedKey={selectedKey} onLinkClick={onLinkClick}>
        <PivotItem headerText="Pipeline drafts" itemKey="drafts">
          ...
          />
          {/* Pagination controls */}
          <div style={{ display: "flex", justifyContent: "center", marginTop: 20  , marginBottom: 20 , gap: 10 }}>
            <DefaultButton
              text="Previous"
